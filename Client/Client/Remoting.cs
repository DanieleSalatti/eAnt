...
namespace eAnt.Client
{
	...
	{
		private TcpClientChannel m_AntChannel;
		...
		{	
			...
			//2ª Opcion			
			provider = new eAntClientSinkProvider(propsinks,datasinks);	
		...
		~CedonkeyCRemoto()
		{
			...
			ChannelServices.UnregisterChannel(m_AntChannel);		
		}
		...
		{			
			...
			try
			{
				m_AntChannel = new TcpClientChannel(props, provider);	
				ChannelServices.RegisterChannel(m_AntChannel);
			}
			catch
			{				
				DisConnect();
				if (m_AntChannel!=null)
					ChannelServices.RegisterChannel(m_AntChannel);
				else
					m_AntChannel = new TcpClientChannel(props, provider);	
			}
			interfazremota = (CInterfaceGateway) Activator.GetObject(typeof(eAnt.eDonkey.CInterfaceGateway),
				this.m_url);
		...
		{			
			...
			{					
				...
				{
					ChannelServices.UnregisterChannel(this.canalCeAnt);
					this.canalCeAnt = null;
					Debug.Write("\nNo se pudo encontrar el servidor\n");
				}*/
			}
			return valor;
	...
	[Serializable]
	public class eAntClientSinkProvider : IClientChannelSinkProvider, IClientFormatterSinkProvider
	{
		...
		public IClientChannelSink CreateSink(IChannelSender channel, string url, object remoteChannelData)
		{
			...
			{
				this.m_NextChannelSink=new eAntClientSink(this,url,NextSink);
				return this.m_NextChannelSink;
			} 
			else 
			{
				return new eAntClientSink(this,url,NextSink);
			}
	...
	[Serializable]
	public class eAntClientSink:BaseChannelObjectWithProperties,IClientChannelSink,IMessageSink
	{
		private string m_url=null;
		private IClientChannelSink m_NextChannelSink=null;
		private IMessageSink m_NextMessageSink=null;
		private eAntClientSinkProvider m_Provider=null;
		private Hashtable m_properties = new Hashtable();
		private eAnt.Classes.Config m_preferences;
		private CompressionType m_CompressionMethod;

		#region Constructor
		public eAntClientSink(IClientChannelSinkProvider Provider, string url)
		{
			object nextobject=new BinaryClientFormatterSink(this);			
			this.m_url=url;
			this.m_Provider=Provider as eAntClientSinkProvider;
			this.m_NextChannelSink = nextobject as IClientChannelSink;
			this.m_NextMessageSink = nextobject as IMessageSink;
			m_preferences = new Config(Application.StartupPath, "config.xml", "0.02", "AntKernel");
			this.m_CompressionMethod=(CompressionType)m_preferences.GetEnum("CompressionMethod",CompressionType.Zip);
		}

		public eAntClientSink(IClientChannelSinkProvider Provider, string url, object nextobject)
		{
			this.m_url=url;
			this.m_Provider=Provider as eAntClientSinkProvider;
			if (nextobject != null)
