...
{
	/// <summary>
	/// Summary description for eAntForm.
	/// </summary>
	public class eAntForm : System.Windows.Forms.Form
	{
		...
		private eAnt.Interface.eAntToggleButton btnDownloads;
		private System.Windows.Forms.Panel panelExtensible;
		private System.Windows.Forms.Panel panelButtons;
		private eAnt.Interface.eAntToggleButton btnServers;
		private eAnt.Interface.eAntToggleButton btnSearch;
		private eAnt.Interface.eAntToggleButton btnUploads;
		private eAnt.Interface.eAntButton buttonOptions;
		...
		private eAnt.Interface.eAntToggleButton btnFriends;
		...
		private System.Windows.Forms.MenuItem menuSupport;
		...
		public eAntForm(string elink, Mutex in_Mutex)
		{
			m_Mutex=in_Mutex;
			m_link=elink;
		...
		public eAntForm(string elink,Mutex in_Mutex,CInterfaceGateway remoteGateway)
		{
			m_Mutex=in_Mutex;
			m_link=elink;
		...
		private void Initialize()
		{
			...
			preferences = new Config(Application.StartupPath, "configInterface.xml", "0.01", "AntInterface");
			...
			toolTipMain.SetToolTip(eAntButtonConnect,Globalization["LBL_CONNDISC"]);
			...
			taskbarNotifier1.SetCloseBitmap(new Bitmap(typeof(eAntForm),"Client.Resources.Classic.close.bmp"),Color.FromArgb(255,0,255),new Point(127,8));
		...
		private void m_ApplySkin()
		{
			...
			eAntButtonConnect.Left=x;
			eAntButtonConnect.Top=y;
			...
			eAntButtonConnect.Width=x;
			eAntButtonConnect.Height=y;
		...
		private void InitializeComponent()
		{
			...
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(eAntForm));
			...
			this.graphics = new eAnt.Interface.eAntGraphic(this.components);
			this.panelToolbarRight = new System.Windows.Forms.Panel();
			this.panelButtons = new System.Windows.Forms.Panel();
			this.eAntButtonConnect = new eAnt.Interface.eAntButton(this.components);
			this.imageListServerCon = new System.Windows.Forms.ImageList(this.components);
			this.buttonOptions = new eAnt.Interface.eAntButton(this.components);
			this.imageListOptions = new System.Windows.Forms.ImageList(this.components);
			this.btnFriends = new eAnt.Interface.eAntToggleButton(this.components);
			this.btnServers = new eAnt.Interface.eAntToggleButton(this.components);
			this.btnSearch = new eAnt.Interface.eAntToggleButton(this.components);
			this.btnUploads = new eAnt.Interface.eAntToggleButton(this.components);
			this.btnDownloads = new eAnt.Interface.eAntToggleButton(this.components);
			...
			this.menuSupport = new System.Windows.Forms.MenuItem();
			...
			this.labelStatusMsg.Text = "Ant v";
			...
			this.panelButtons.Controls.Add(this.eAntButtonConnect);
			...
			// eAntButtonConnect
			// 
			this.eAntButtonConnect.BackColor = System.Drawing.Color.Transparent;
			this.eAntButtonConnect.Cursor = System.Windows.Forms.Cursors.Hand;
			this.eAntButtonConnect.FlatStyle = System.Windows.Forms.FlatStyle.Flat;
			this.eAntButtonConnect.ImageIndex = 1;
			this.eAntButtonConnect.ImageList = this.imageListServerCon;
			this.eAntButtonConnect.Location = new System.Drawing.Point(360, 8);
			this.eAntButtonConnect.Name = "eAntButtonConnect";
			this.eAntButtonConnect.Size = new System.Drawing.Size(40, 40);
			this.eAntButtonConnect.TabIndex = 6;
			this.eAntButtonConnect.Click += new System.EventHandler(this.eAntButton1_Click);
			// 
			// imageListServerCon
			// 
			...
			this.menuItemWeb.Text = "Ant Web";
			this.menuItemWeb.Click += new System.EventHandler(this.menuItemAbout_click);
			// 
			// menuItem2
			// 
			this.menuItem2.Index = 1;
			this.menuItem2.Text = "Skins";
			this.menuItem2.Click += new System.EventHandler(this.menuItem2_Click);
			// 
			// menuSupport
			// 
			this.menuSupport.Index = 2;
			this.menuSupport.Text = "Request Support";
			this.menuSupport.Click += new System.EventHandler(this.menuItem3_Click);
			// 
			// eAntForm
			// 
			this.AllowDrop = true;
			...
			this.Name = "eAntForm";
			this.Text = "eAnt v";
			this.Resize += new System.EventHandler(this.eAntForm_Resize);
			this.Closing += new System.ComponentModel.CancelEventHandler(this.eAntForm_Closing);
			this.Load += new System.EventHandler(this.eAntForm_Load);
			this.DragOver += new System.Windows.Forms.DragEventHandler(this.eAntForm_DragOver);
			this.DragDrop += new System.Windows.Forms.DragEventHandler(this.eAntForm_DragDrop);
			this.Activated += new System.EventHandler(this.eAntForm_Activated);
		...
		private void eAntForm_Closing(object sender, System.ComponentModel.CancelEventArgs e)
		{
			if (MessageBox.Show(Globalization["MSG_EXIT"],"Ant",MessageBoxButtons.YesNo,MessageBoxIcon.Warning)==DialogResult.Yes)
			{
				m_Close_eAnt();
			}
			else e.Cancel=true;
		...
		private void m_OnRefreshStatus(CkernelGateway in_krnGateway)
		{
			...
			labelSessionUP.Text=Globalization["LBL_UPLOADED"]+":"+eAntListView.SizeToString(status.SessionUpload);
			labelSessionDO.Text=Globalization["LBL_DOWNLOADED"]+":"+eAntListView.SizeToString(status.SessionDownload);
			notifyIcon1.Text=labelStatusUPSpeed.Text+" "+labelStatusDOSpeed.Text+" "+labelStatusAvgDOSpeed.Text;
			this.Text= String.Format("eAnt v{0} ( {1} ) ", in_krnGateway.Version, notifyIcon1.Text);
#if DEBUG
			...
			if (status.UserID!=0)
				eAntButtonConnect.ImageList=this.imageListServerDiscon;
			else
				eAntButtonConnect.ImageList=this.imageListServerCon;
		}
		...
		private void eAntForm_Activated(object sender, System.EventArgs e)
		{
/*			if (!m_KernelLoaded)
			{
				FDownloads.Show();
...
*/		}
		...
		private void eAntForm_Resize(object sender, System.EventArgs e)
		{
		...
		private void eAntForm_Load(object sender, System.EventArgs e)
		{
		...
		/// <summary>
		/// Checks if the Kernel configuration file already exists
		/// If it doesn't then the default configuration is given to the wizard,
		/// in orden to help the user override the default preferences he wants.
		/// </summary>
		private void CheckWizardNeeded()
		{
			string configFile = CKernel.DllDirectory + "config.xml";
			if (!File.Exists(configFile))
			{
				InterfacePreferences defaultPreferences = krnGateway.GetConfig();
				//By default autoconnect should be true.
				defaultPreferences.Autoreconect = true;
				
				//There is no previous config, let's launch the wizard
				WizardForm wizard = new WizardForm(defaultPreferences);
				wizard.ShowDialog();
				
				InterfacePreferences preferences= wizard.GetPreferences();
				//The preferences not entered by the user should remain
				//untouched, so we asign all.
				krnGateway.SetConfig(preferences);
				m_Preferences = preferences;

				if (preferences.Autoreconect)
				{
					krnGateway.ConnectToaServer();
				}

				//The transfer limits could have been changed
				UpdateGraphScale();
			}
		}
		...
		private void loadAltConnections()
		{
			...
			altConnectionsUp	= eAntForm.preferences.GetStringArray("altConnectionsUp");
			altConnectionsDown	= eAntForm.preferences.GetStringArray("altConnectionsDown");
			...
			{			
				...
				eAntForm.preferences.SetProperty("altConnectionsUp",altConnectionsUp);
				eAntForm.preferences.SetProperty("altConnectionsDown",altConnectionsDown);
			}
...
//				taskbarNotifier1.Show("",labelStatusMsg.Text,500,3000,500);
		...
		private void m_Globalize()
		{
			...
			contextMenuAbout.MenuItems[0].Text=Globalization["LBL_AntWEB"];
			contextMenuAbout.MenuItems[1].Text="Skins";//Globalization["LBL_AntWEB"];
			contextMenuAbout.MenuItems[2].Text=Globalization["LBL_HELP"]+"/"+Globalization["LBL_SUPPORT"];
		...
		{
			System.Diagnostics.Process.Start("http://www.salatti.net/eAnt");
		}
		...
		public void CercaVersione()
		{
			try
			{
				Classes.Version Loader=new eAnt.Classes.Version();
				Loader.LoadConfig("http://www.salatti.net/eAnt/UpdateVersion.xml");
				string Attuale;
				string Nuova;
				Attuale=Loader.ComparatoreDiVersione.Actual.Version;
				Nuova=Loader.ComparatoreDiVersione.New.Version;
				if (Attuale!=Nuova)
				{
					string message = "New version available...\nDo you want to download the newer version?";
					string caption = "Version alert...";
					...
					if(result == DialogResult.Yes)
					{
						...
						System.Diagnostics.Process.Start(Loader.ComparatoreDiVersione.New.URL);
		...
		private void eAntForm_DragOver(object sender, System.Windows.Forms.DragEventArgs e)
		{
		...
		private void eAntForm_DragDrop(object sender, System.Windows.Forms.DragEventArgs e)
		{
			if (e.Data.GetDataPresent(DataFormats.Text))
			{
				// get data
		...
		{
		System.Diagnostics.Process.Start("http://sourceforge.net/tracker/?func=add&group_id=137079&atid=738797");
		}

		private void menuItem2_Click(object sender, System.EventArgs e)
		{
		System.Diagnostics.Process.Start("http://www.salatti.net/eAnt/Skins");
		}
