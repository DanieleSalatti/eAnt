...
namespace eAnt.Client
{
	...
	public class eAntListView : ListView
	{
		...
		public void SavePreferences()
		{
			...
			{
				eAntForm.preferences.SetProperty(Name+"ColumnWidth-"+i.ToString(),GetColumnsWidth(i));
				eAntForm.preferences.SetProperty(Name+"ColumnPosition-"+i.ToString(),this.GetColumnPosition(i));
			}
		}
		...
		protected void m_LoadWidths()
		{
			...
			for (int i=0;i<Columns.Count;i++)
			{
				...
				Columns[i].Width=eAntForm.preferences.GetInt(Name+"ColumnWidth-"+i.ToString(),def);
			}
		}
		private void m_LoadPreferences()
		{
			...
			for (int i=0;i<Columns.Count;i++)
			{
				newPosition=eAntForm.preferences.GetInt(Name+"ColumnPosition-"+i.ToString(),i);
...
//														new Point(img.Left,img.Top)}); 
		...
		internal class HeaderControl : NativeWindow
		{
			eAntListView parent;
			bool mouseDown;
			public HeaderControl(eAntListView m)
			{
				parent = m;
	...
	#region transformationRoutines
		...
		public static string SizeToString(ulong size)
		{
			...
			{ 
				strB.Append(size);
				strB.Append(" "+eAntForm.Globalization["LBL_BYTE"]);
			}
			else if ((size>=1024)&&(size<1048576)) 
			{
				strB.Append(size/1024);
				strB.Append(" "+eAntForm.Globalization["LBL_KBYTE"]);
			}
			else if ((size>=1048576)&&(size<1073741824))
			{
				strB.Append(((float)size/1048576F).ToString("###0.##"));
				strB.Append(" "+eAntForm.Globalization["LBL_MBYTE"]);
			}
			else if (size>=1073741824) 
			{
				strB.Append(((float)size/1073741824F).ToString("###0.##"));
				strB.Append(" "+eAntForm.Globalization["LBL_GBYTE"]);
			}
...
//			return"";
			...
			{
				strB.Append(speed);
				strB.Append(" "+eAntForm.Globalization["LBL_KBYTE"]+"/s");
			}
			else 
			{
				strB.Append(speed/1024);
				strB.Append(" "+eAntForm.Globalization["LBL_MBYTE"]+"/s");
			}
			return strB.ToString();
		}
		protected string PriorityToString(Types.Constants.Priority priority)
		{
			switch (priority)
			{
				case Types.Constants.Priority.VeryHigh: return eAntForm.Globalization["LBL_VERYHIGH"];
				case Types.Constants.Priority.High: return eAntForm.Globalization["LBL_HIGH"];
				case Types.Constants.Priority.Normal: return eAntForm.Globalization["LBL_NORMAL"];
				case Types.Constants.Priority.Low: return eAntForm.Globalization["LBL_LOW"];
				case Types.Constants.Priority.VeryLow: return eAntForm.Globalization["LBL_VERYLOW"];
			}
			return "";
		}
		protected string ServerPriorityToString(Types.Constants.ServerPriority priority)
		{
			switch (priority)
			{
				case Types.Constants.ServerPriority.High: return eAntForm.Globalization["LBL_HIGH"]; 
				case Types.Constants.ServerPriority.Normal: return eAntForm.Globalization["LBL_NORMAL"];
				case Types.Constants.ServerPriority.Low: return eAntForm.Globalization["LBL_LOW"];
			}
			return "";
		}
	#endregion
