	...
	internal class CFile
	{
		...
		public CFile(string in_m_FileFolder, string in_filename)
		{
			...
			{	
				...
				//if (Gaps.Count>0) m_RepairAntOldGaps();				
				m_LoadChunkStatus();
				//m_LoadCrumbsStatus();
		...
		private void m_AddGap(uint start, uint end)
		{
			...
			if (end==m_FileSize-2) end=m_FileSize-1; //for compatibility with old Ant format::: remove for next releases
		...
		private void m_RepairAntOldGaps()
		{
			bool oldFormat;
			oldFormat=false;
...
*/
		...
		private void m_TryToRecoverCorruptedPartCCH(int chunkNumber)
		{
			...
			CLog.Log(Constants.Log.Info,"Trying to recover corrupted part with CCH "+m_FileName);
			byte[] content;
			...
			for (uint i=0;i<currentCrumbs.Count;i++)
			{
				...
				{
					CLog.Log(Constants.Log.Info,"Crumb "+i.ToString()+" in chunk "+chunkNumber.ToString()+" is WRONG");
				}
			}
			...
			{
			CLog.Log(Constants.Log.Info,"Unable to recover parts...",this.Name);
			}
			...
			lock(this)
			{
				...
				if ((Protocol.ChunkState)m_ChunksStatus[chunk] == Protocol.ChunkState.Empty) 
				{
					...
					else
					{
						...
						m_ChunksStatus[chunk]=(byte)Protocol.ChunkState.Downloading;
