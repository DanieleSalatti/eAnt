...
//				writer.Write(value.ToCharArray());
	...
	public struct CReceiveHello
	{
		...
		public CReceiveHello(bool bRespuesta,MemoryStream buffer)
		{
			...
			if ((IDClientCompatible==(byte)Protocol.Client.eAnt)&&(VersioneMuleMajor>0)) VersioneMuleMajor--;
			...
			if ((software==(byte)Protocol.Client.eAnt)&&(Version>1.4))
			{
...
//			buffer=null;
	...
	internal class CMuleHello
	{
		...
		public CMuleHello(MemoryStream buffer,bool isResponse)
		{
			...
			// client compatible
			new ParameterWriter(Protocol.ET_COMPATIBLECLIENT,(byte)Protocol.Client.eAnt,writer);
			// version Ant
			new ParameterWriter(Protocol.ET_eAntVERSION,Protocol.eAnt_VERSION,writer);
			...
			for (int i=0;i!=nParametros;i++)
			{
				...
				switch(ParameterReader.id)
				{
					...
					case Protocol.ET_COMPATIBLECLIENT:
						...
						break;
					case Protocol.ET_eAntVERSION:
						VersionAnt = ParameterReader.valorNum & 0x00FFFFFF;
						break;
				}
			}
