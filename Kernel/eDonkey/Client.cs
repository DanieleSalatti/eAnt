...
namespace eAnt.eDonkey
{
	...
	internal class CClient
	{
		...
		public void ProcessChatMessage(byte[] packet)
		{
			...
			if (m_FirstMessage && ClientMessage.Message.ToLower().IndexOf("viagra")>=0) m_IsSpammer=true;
			if (m_FirstMessage && ClientMessage.Message.ToLower().IndexOf("cialis")>=0) m_IsSpammer=true;
...
//				searchID=(int)CKernel.Searchs.GetKey(CKernel.Searchs.Count-1)+1;
		...
		public void ProcessEmuleHello(byte[] packet,bool isResponse)
		{
			...
			if (m_Software==(byte)Protocol.Client.eAnt)
			{
				m_VersionMinor=MuleHello.VersionAnt;
			}
